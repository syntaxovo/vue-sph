...
export default {
  ...
  methods: {
    ...
    async open () {
      // 生成二维码地址
      const url = await QRCode.toDataURL(this.userpayinfo.codeUrl)
      // console.log(res)
      this.$alert(`<img src=${url} />`, '请你微信支付', {
        ...
        beforeClose: (type, instance, done) => { // MessageBox 关闭前的回调，会暂停实例的关闭
          // type: 区分按钮是取消 cancel ||确认 confirm
      ...
      // 需要知道支付的状态
      // 如果未支付需要一直去发请求查询支付的状态
      if (!this.timer) {
        // 开启定时器
        this.timer = setInterval(async () => {
          const res = await this.$API.lookorderinfoAPI(this.orderId)
          console.log(res)
          if (res.code === 200) {
            // 支付成功，
            // 先清除定时器
