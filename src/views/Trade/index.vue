  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="(add) in address" :key="add.id">
        ...
        <p @click="changeactive(add,address)">
    ...
    <div class="money clearFix">
      <ul>
        <li>
          <b><i>{{orderinfo.totalNum}}.00</i>件商品，总商品金额</b>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{mainuserinfo.userAddress}}</span>
        收货人：<span>{{mainuserinfo.consignee}}</span>
        <span>{{mainuserinfo.phoneNum}}</span>
      </div>
    </div>
    <div class="sub clearFix">
       <a class="subBtn" @click="tosubmit">提交订单</a>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted () {
    // 在页面刚挂载时请求用户信息
    this.$store.dispatch('getdorderinfo')
    // this.$store.trade.state.address()
  },
  methods: {
    changeactive (add, address) {
      // 改变地址信息的选中状态
      // 先把列表所有项状态都变为为选中
